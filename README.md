...
- **데이터 세팅**: 정산월 선택 + 필수 4개 파일(카카오 빌링 / SAP / 카카오 현금영수증 / AXZ 내부결제) + PG 상세(선택) 업로드 후 분석
- **정산 대시보드**: 총괄 요약, AXZ 정산 피벗(결제완료·취소완료·부분취소), PG / 현금영수증 / 세금계산서 대사, 정산 제외(구다음) 내역
- **엑셀 다운로드**: 결과를 수식이 적용된 .xlsx 로 내려받기
  - 원본 시트: `원본_AXZ` / `원본_SAP` / `원본_카카오빌링` / `원본_현금영수증` / `원본_PG`
  - `원본_AXZ` 에 보조 수식열(정산월여부 / 최종매출인식금액 / 구다음 판별 / 대상매출) 추가
  - 요약 시트: `요약_AXZ정산` / `요약_PG` / `요약_현금영수증` / `요약_세금계산서` / `정산제외`
  - AXZ정산·세금계산서·PG·현금영수증 원본합계는 `SUMIFS/SUMIF` 수식이라 **원본을 고치면 요약이 자동 재계산**됩니다.
